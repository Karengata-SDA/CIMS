    ...
    {        
        if($endpoint == "login" || $endpoint == "members" || $endpoint == "register" || $endpoint == "profile" || $endpoint == "departments" || $endpoint == "accounts" || $endpoint == "collections" || $endpoint == "verification" || $endpoint == "search" || $endpoint == "message" || $endpoint == "messages" || $endpoint == "statements" || $endpoint == "products" || $endpoint == "revenues" || $endpoint == "transaction" || $endpoint == "transactions" || $endpoint == "dashboard")
        {
            ...
            elseif($endpoint == "transaction")
            {
                ...
                $msg = mysqli_real_escape_string($conn, "Dear ".$fname.",\nYour giving of Ksh ".$total." has been received as follows:\n\n".$bag."\nBe Blessed,\nKarengata Treasury.");
                ...
                $queryInSms = "INSERT INTO `messages` (id, `from`, `to`, `message`, `reference`, `status`, `reg_timestamp`) VALUES (NULL, '$from', '$to', '$msg', '$str_trxno', '$respmsg', CURRENT_TIMESTAMP);";
                mysqli_query($conn, $queryInSms);//  or die(mysqli_error($conn)."\n\n".$queryInSms);
                
                //$query = mysqli_query($conn, "INSERT INTO `messages` (`id`,`voornaam`, `achternaam`, `email`, `telefoonnummer`) values ('','".$voornaam."', '".$achternaam."', '".$email."', '".$telefoonnummer."')") or die(mysqli_error($mysqli));
            ...
            elseif($endpoint == "message")
            {
                ...
                $msg = mysqli_real_escape_string($conn, $obj['parameter4']);
                ...
                if($type == "multiple" || $type == "all")
                {
                    ...
                    for($i=0; $i<$count; $i++)
                    {
                        ...
                        if($user != "")
                        {
                            ...
                            //$ujumbe = mysqli_real_escape_string($conn, "As you give this Sabbath, clearly write '".$trxno."' on the envelope. This no. will be used for receipting and tracking purposes.\n\nBe Blessed,\nTreasury Dept.");
                            //$ujumbe = mysqli_real_escape_string($conn, "We have been piloting the Treasury's SMS reporting these last Sabbaths with the Church Board Members.\n\nAs we prepare to roll it out to the rest of the members, we would like your feedback as suggestions or observations e.g. any errors in messaging etc on the last two Sabbaths' notifications.\n\nPlease reach me at 0720115358 with feedback!\nGeorge,\nChurch Treasurer.\n\nUse '".$trxno."' as your transaction number this Sabbath.");
