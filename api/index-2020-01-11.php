                    ...
                    for($i=0; $i<$numb; $i++)
                    {
                        ...
                        if($type == "giving")
                        {
                            ...
                            if($item != "")
                            {
                                ...
                                $item_cab = explode("^", $item[1]); //$item[1] 
                                $item_co = $item_cab[0];
                                $item_nu = $item_cab[1];
                                $item_qt = $item_cab[2];
                                $item_id = $item_cab[3];
                ...
                if($type == "multiple" || $type == "all")
                {
                    ...
                    for($i=0; $i<$count; $i++)
                    {
                        ...
                        if($user != "")
                        {
                            ...
                            //$ujumbe = "Welcome to the first Sabbath of the year 2020. Take heed to the words of Isaiah 43:18,19 where God says... 'Do not cling on past events or dwell on the things of old. Behold I will do a new thing.'\n\nToday is a prayer & fasting Sabbath!\n\nFor Care & Concerns: 020 200 88 88\nYour Member No is ".$memnu."\n";
                            $ujumbe = "Karibu to 10 days of prayer themed SEEKING GOD's SPIRIT!\n\nMeetings will be held from 5:30pm to 7pm starting today.\n\nSMS/Call: 020 200 88 88\n";
                            //$ujumbe = mysqli_real_escape_string($conn, $ujumbe);
                            try
                            {
                                $results = $gateway->sendMessage($to, "Dear ".$fname.",\n".$ujumbe, $from); //$to, $ujumbe, $from
