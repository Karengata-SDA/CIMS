...
if($request_method == "POST")
{
    ...
    {        
        if($endpoint == "authenticate" || $endpoint == "settings" || $endpoint == "programme" || $endpoint == "login" || $endpoint == "members" || $endpoint == "registration" || $endpoint == "rescue" || $endpoint == "profile" || $endpoint == "departments" || $endpoint == "accounts" || $endpoint == "collections" || $endpoint == "verification" || $endpoint == "search" || $endpoint == "message" || $endpoint == "messages" || $endpoint == "statements" || $endpoint == "products" || $endpoint == "revenues" || $endpoint == "transaction" || $endpoint == "transactions" || $endpoint == "dashboard" || $endpoint == "verification_mpesa")
        {
            if($endpoint == "login")
            {
                $username = $parameter2;
            ...
            elseif($endpoint == "rescue")
            {
                $credential = $parameter2; //trim(substr($parameter2, -9)); //
                
                $query = "SELECT * FROM `userdetails` WHERE `email` = '$credential'"; //`phone` = '$phoneno' OR     //`id`, `trx_number`, `member_no`, `firstname`, `lastname`, `username`, `password`, `email`, `phone`
                $results = mysqli_query($conn, $query);
                ...
                if($numrows == 1) 
                {
                    ...
                    $query = "UPDATE `userdetails` SET  `password` = '$otpcode' WHERE `email` = '$credential'"; // AND `message_id` = '".$messg_id."' AND `from_id` = '$udid' AND  `to_id` = '$recmemno'
                    $update = mysqli_query($conn, $query);
                    
                    $subject = "Login Rescue";
                    ...
                    //
                        ...
                        $mail->AddAddress($credential, $firstname." ".$lastname);
                        ...
                        <br>Dear ".$firstname.",<br>Your ".$subject." has successfuly been done and your login credentials are as shown below:<br><br>
                        <br><b>Username:</b> " . $credential . "<br>
                        <br><b>Password:</b> Sent to your phone number " . $phoneno . "<br><br>
            ...
            elseif($endpoint == "transaction")
            {
                ...
                */
                ...
                else 
                {
                    ...
                    $lname = $parameter6; $name = ucwords(mysqli_real_escape_string($conn, $fname." ".$lname), "");  //$name = $fname." ".$lname;
