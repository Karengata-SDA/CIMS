    ...
    {        
        if($endpoint == "authenticate" || $endpoint == "settings" || $endpoint == "programme" || $endpoint == "login" || $endpoint == "members" || $endpoint == "register" || $endpoint == "profile" || $endpoint == "departments" || $endpoint == "accounts" || $endpoint == "collections" || $endpoint == "verification" || $endpoint == "search" || $endpoint == "message" || $endpoint == "messages" || $endpoint == "statements" || $endpoint == "products" || $endpoint == "revenues" || $endpoint == "transaction" || $endpoint == "transactions" || $endpoint == "dashboard" || $endpoint == "verification_mpesa")
        {
            ...
            elseif($endpoint == "programme")
            {
                if($parameter3 != "")
                {
                    $query = "UPDATE `programmes` SET  `activity` = '$parameter4', `comments` = '$parameter5',  `start_time` = '$parameter6', `end_time` = '$parameter7', `date_scheduled` = '$parameter8',  `presenters` = '$parameter9',  `location` = '$parameter10', `department` = '$parameter11', `updated_by` = '$parameter12' WHERE `programmes`.`id` = '$parameter3'"; // AND `message_id` = '".$messg_id."' AND `from_id` = '$udid' AND  `to_id` = '$recmemno'
                    $update = mysqli_query($conn, $query);
                    
                    $query2 = "SELECT * FROM `programmes` WHERE `date_scheduled` = '$parameter2';";
