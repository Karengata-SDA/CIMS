    ...
    {        
        if($endpoint == "authenticate" || $endpoint == "login" || $endpoint == "members" || $endpoint == "register" || $endpoint == "profile" || $endpoint == "prayer" || $endpoint == "departments" || $endpoint == "accounts" || $endpoint == "collections" || $endpoint == "verification" || $endpoint == "search" || $endpoint == "message" || $endpoint == "messages" || $endpoint == "statements" || $endpoint == "products" || $endpoint == "revenues" || $endpoint == "transaction" || $endpoint == "transactions" || $endpoint == "dashboard" || $endpoint == "verification_mpesa")
        {
            ...
            elseif($endpoint == "prayer")
            {
                    ...
                    $sql = "INSERT INTO `messages` (`id`, `message_id`, `conversation_id`,  `organisation_id`,    `from`,   `to`,   `from_name`,   `to_name`,  `from_id`,  `to_id`,     `message`,       `box`,         `source`,         `reference`,     `cost`,      `status`,   `added_on`,   `added_at`) VALUES "
                                                 ."(NULL,  '$respidn',         '0',             '$orgid',         '$from',  '$to',  '$name',         '$name',   '$udid',   '$memno',     '$msg',       '$subject',     'mobile_app',      '$reference',  '$respcst',   '$respmsg',  '$leo_date',  '$leo_time')";
...
Please remember to attend our meeting this evening at 8PM. 

See AMO forum for more details.
